...
function showOverlay(id) {
	...
	const styleString = elm.getAttribute("style");
...
function _main_events() {
	var header_bar_1 = document.querySelector(".header_bar_1");
	if (header_bar_1 == null) header_bar_1 = document.querySelector("#header_bar_1");
...
function _showProfile(user) {
	...
	const profilePic = elm.querySelector(".profile-pic");
	if (hasProfilAbility) {
		profilePic.style.setProperty("background-image", `url('${user.avatar}')`);
		elm.querySelector("input").setAttribute("value", user.avatar);
		avatarInp.style.display = "unset";
	} else {
		if (features.includes("pink_mode")) {
			profilePic.style.setProperty("background-image", `url("https://static.turtle-coding-gbr.de/gmci/rosa_profil.png")`);
		} else {
			profilePic.style.setProperty("background-image", `url("https://static.turtle-coding-gbr.de/gmci/standart_profil.png")`);
		}
...
function setXPBarPercent(percent) {
	percent = 100 - percent;
	const elms = document.querySelectorAll(".mc-xp-bar");
	for (var elm of elms) {
		const div = elm.querySelector("div");
		div.style.setProperty('clip-path', `inset(0 ${percent}% 0 0)`);
	}
}