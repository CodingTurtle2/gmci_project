...
class CouchDB {
	...
	_does_auth_exists_err() {
		if (!this._does_auth_exists()) throw 'Keine Auth vorhanden. Digga, mach mal <script src="./auth.js"></script>';
	}
	_is_error_missing_database(errReason) {
		return errReason === "no_db_file" || errReason === "Database does not exist.";
	}
	...
	_send_db_request(method, url, body = null, errorHandling = true) {
		return new Promise(async (resolve, reject) => {
				...
				try {
					...
					//errors
						...
						if (this._is_error_missing_data(json.reason)) {
							resolve(null);
						} else if (errorHandling && this._is_error_missing_database(json.reason)) {
							this.createDB().then((value) => {
								this._send_db_request(method, url, body, false).then((value) => {
									resolve(value);
								}).catch((err) => {
									reject(err);
								});
							...
							reject(`Database not found: ${request.status} (Reason: ${reason.json})`);
						}
					} else {
						reject(`Unexpected status: ${request.status} (Response: ${
			...
			this.request.setRequestHeader("Authorization", "Basic " + btoa(
	...
	async set(id, content, replace = true) {
		try {
			if (typeof (content) != "object") throw "basic.js - CouchDB->set: content is not from type object (type = " + typeof(content) + ")";
			...
			const res = await this._send_db_request("PUT", id, JSON.stringify(content));
			return {
				"type": "success",
				"message": res
			};
